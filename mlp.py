...
def label_encode(label):
	...
	if label == "nome":
		...
		val = [0,1,0,0,0,0] 
	elif label == "dtexpedicao":
		val = [0,0,1,0,0,0]
	elif label == "filiacaomae":
		val = [0,0,0,1,0,0] 
	elif label == "filiacaopai":
		val = [0,0,0,0,1,0]
	elif label == "dtnascimento":
		val = [0,0,0,0,0,1]
	return val
...
def data_encode(file):
	...
	for line in train_file.read().strip().split('\n'):
		line = line.split(',')
		X.append([line[0],line[1],line[2],line[3],line[4],line[5]])
		Y.append(label_encode(line[6]))
	return X,Y
...
train_X, train_Y = data_encode("mlp.train")
test_X, test_Y = data_encode("mlp.test")
...
with tf.Session() as sess:
	...
	for epochs in range(training_epochs):
		...
		custodeaprendizagem.append(c)
		if(epochs + 1) % display_steps == 0:
			print("Epoch:",epochs+1,"Cost:", c)
	print("Optimization Finished")

	plt.plot([np.mean(custodeaprendizagem[i]) for i in range(len(custodeaprendizagem))])
	plt.show()
	plt.savefig("custodeaprendizagem.png")
	...
	print('-------')
	print("tf.equal:", correct_prediction)
